create or replace function public.get_my_exam_attempt_evidence(p_attempt_id uuid)
returns jsonb
language plpgsql
stable
security definer
...
begin
  ...
  if v_attempt.id is null then
    raise exception 'Evidência da avaliação não encontrada';
  end if;
  ...
  for v_question in select value from jsonb_array_elements(coalesce(v_exam.questions, '[]'::jsonb)) loop
    ...
    v_items := v_items || jsonb_build_array(jsonb_build_object(
      'id', coalesce(v_question->>'id', 'q-' || v_total::text),
      'order', v_total,
      'type', v_type,
  ...
  return jsonb_build_object(
    ...
    'certificate_code', v_attempt.certificate_code,
    'finished_at', v_attempt.finished_at,
    'signed_at', v_attempt.signed_at,
    'signature_name', v_attempt.signature_name,
...
revoke all on function public.get_my_exam_attempt_evidence(uuid) from public;
grant execute on function public.get_my_exam_attempt_evidence(uuid) to authenticated;
