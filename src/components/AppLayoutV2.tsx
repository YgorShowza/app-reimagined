...
function MenuLink({ item }: { item: MenuItem }) {
  ...
  return <Link to={item.path} className="block" activeOptions={{ exact: item.path === "/admin" || item.path === "/painel" }}>{({ isActive }) => <div className="segempat-sidebar-link relative flex items-center gap-3 rounded-xl px-3.5 py-2.5 transition-all duration-150" data-active={isActive ? "true" : "false"} style={isActive ? { background: "linear-gradient(135deg,#e0142f,#C8102E)", border: "1px solid rgba(255,84,112,.34)", boxShadow: "0 8px 20px rgba(200,16,46,.18)" } : { border: "1px solid transparent" }}><Icon className="h-[17px] w-[17px] shrink-0" style={{ color: isActive ? "#fff" : "var(--text-3)" }} /><span className="text-[13px] font-semibold tracking-wide" style={{ color: isActive ? "#fff" : "var(--text-2)" }}>{item.label}</span></div>}</Link>;
}
...
function SidebarFooter({ loggingOut, onLogout }: { loggingOut: boolean; onLogout: () => void }) {
  return <div className="space-y-3 px-3 pb-4 pt-3" style={{ borderTop: "1px solid var(--border)" }}><div><div className="mb-1.5 flex items-center gap-1.5 px-1"><Palette className="h-3.5 w-3.5" style={{ color: "var(--text-4)" }} /><span className="text-[9px] font-black uppercase tracking-[.14em]" style={{ color: "var(--text-4)" }}>Tema da interface</span></div><ThemeToggle /></div><button disabled={loggingOut} onClick={onLogout} className="flex w-full items-center gap-3 rounded-xl px-3.5 py-2.5 transition-colors duration-150 disabled:opacity-50" style={{ border: "1px solid rgba(200,16,46,.26)", background: "var(--accent-soft)" }}><LogOut className="h-[17px] w-[17px]" style={{ color: "var(--accent)" }} /><span className="text-[13px] font-semibold" style={{ color: "var(--accent)" }}>{loggingOut ? "Saindo..." : "Sair do sistema"}</span></button></div>;
}
