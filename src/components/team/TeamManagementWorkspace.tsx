...
import { Eye, KeyRound, LockKeyhole, Pencil, Plus, Radio, Search, ShieldCheck, Trash2, UserRoundCog, Users, UserX } from "lucide-react";
...
export function TeamManagementWorkspace() {
  ...
  const availableSectors = useMemo(() => ["Todos", ...Array.from(new Set(employees.map((employee) => employee.sector))).sort((a, b) => a.localeCompare(b, "pt-BR"))], [employees]);
  ...
  const filtered = useMemo(() => {
    ...
    return employees.filter((employee) => {
      ...
      return !query || employee.full_name.toLowerCase().includes(query) || employee.matricula.includes(query) || employee.sector.toLowerCase().includes(query);
    });
  ...
  const save = useMutation({
    ...
    onError: (error: Error) => toast.error(error.message.includes("duplicate") ? "Matrícula já cadastrada" : error.message),
  });
    ...
    <div className="mx-auto w-full max-w-[1536px] space-y-5 pb-10">
      <section className="relative overflow-hidden rounded-[1.75rem] p-5 md:p-6 xl:p-7" style={{ background: "linear-gradient(135deg,#171117 0%,#310912 55%,#160f14 100%)", border: "1px solid rgba(200,16,46,.28)", boxShadow: "0 12px 38px rgba(80,0,18,.16)" }}>
        ...
        <div className="relative flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
          <div><div className="flex items-center gap-2 text-[10px] font-black uppercase tracking-[.22em]" style={{ color: "rgba(255,255,255,.44)" }}><UserRoundCog className="h-4 w-4" /> Gestão operacional</div><h1 className="mt-2 text-2xl font-black tracking-tight text-white md:text-3xl xl:text-[2.1rem]">Gestão de Equipe</h1><p className="mt-1 text-sm" style={{ color: "rgba(255,255,255,.52)" }}>{employees.length} cadastrados · {cadastrosAtivos} ativos · {inativos} inativos</p></div>
          {isAdmin && <div className="flex flex-wrap gap-2"><Button asChild variant="outline" className="border-white/15 bg-white/5 font-bold text-white hover:bg-white/10 hover:text-white"><Link to="/acessos"><KeyRound className="mr-2 h-4 w-4" />Acessos</Link></Button><Button onClick={openNew} className="bg-[#e0142f] font-bold text-white shadow-lg shadow-red-950/20 hover:bg-[#C8102E]"><Plus className="mr-2 h-4 w-4" />Novo Funcionário</Button></div>}
        </div>
      </section>
      ...
      <Surface className="p-4 xl:p-5">
        <div className="flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
          <div><p className="text-sm font-black" style={{ color: "var(--text-1)" }}>Funcionários</p><p className="mt-0.5 text-[11px]" style={{ color: "var(--text-4)" }}>{filtered.length} de {employees.length} registros exibidos</p></div>
        ...
        <div className="mt-4 grid gap-3 lg:grid-cols-[minmax(320px,1fr)_220px_180px]">
          <div className="relative"><Search className="absolute left-3 top-1/2 h-4 w-4 -translate-y-1/2" style={{ color: "var(--text-4)" }} /><Input value={search} onChange={(event) => setSearch(event.target.value)} placeholder="Buscar por nome, matrícula ou setor..." className="pl-10" /></div>
          <Select value={sectorFilter} onValueChange={setSectorFilter}><SelectTrigger><SelectValue /></SelectTrigger><SelectContent>{availableSectors.map((sector) => <SelectItem key={sector} value={sector}>{sector === "Todos" ? "Todos os setores" : sector}</SelectItem>)}</SelectContent></Select>
          <Select value={statusFilter} onValueChange={setStatusFilter}><SelectTrigger><SelectValue /></SelectTrigger><SelectContent><SelectItem value="Todos">Todas as situações</SelectItem>{SITUACOES.map((status) => <SelectItem key={status} value={status}>{status}</SelectItem>)}</SelectContent></Select>
        </div>
      </Surface>

      {isLoading ? <div className="flex justify-center p-12"><div className="h-8 w-8 animate-spin rounded-full border-4" style={{ borderColor: "var(--border)", borderTopColor: "#C8102E" }} /></div> : filtered.length === 0 ? (
        <Surface className="p-9 text-center"><div className="mx-auto mb-4 flex h-14 w-14 items-center justify-center rounded-2xl" style={{ background: "#C8102E1a" }}><Users className="h-6 w-6 text-[#C8102E]" /></div><p className="font-bold" style={{ color: "var(--text-1)" }}>Nenhum funcionário encontrado</p><p className="mt-1 text-sm" style={{ color: "var(--text-3)" }}>Ajuste os filtros para visualizar outros registros.</p></Surface>
      ) : (
        <>
              ...
              <table className="w-full min-w-[920px] border-collapse text-left">
                <thead className="sticky top-0 z-10" style={{ background: "var(--bg-surface)" }}>
                  <tr style={{ borderBottom: "1px solid var(--border)" }}>
                    {['Funcionário','Matrícula','Setor','Perfil','Situação','Ações'].map((label) => <th key={label} className={`px-4 py-3 text-[10px] font-black uppercase tracking-[.13em] ${label === 'Ações' ? 'text-right' : ''}`} style={{ color: "var(--text-4)" }}>{label}</th>)}
                  </tr>
                </thead>
                <tbody>
                  {filtered.map((employee) => {
                    ...
                    return (
                      <tr key={employee.id} className="transition-colors hover:bg-black/[.015] dark:hover:bg-white/[.025]" style={{ borderBottom: "1px solid var(--border-subtle)" }}>
                        ...
                        <td className="px-4 py-3.5"><div className="flex justify-end gap-1">{isAdmin && <><Button variant="ghost" size="icon" title={privileged ? "Editar dados não privilegiados" : "Editar"} onClick={() => openEdit(employee)}><Pencil className="h-4 w-4" /></Button>{!privileged && <Button variant="ghost" size="icon" title="Excluir" className="text-red-500 hover:text-red-600" onClick={() => setToDelete(employee)}><Trash2 className="h-4 w-4" /></Button>}</>}</div></td>
                      </tr>
                    );
                  })}
                </tbody>
          ...
          <div className="grid gap-3 lg:hidden">
            {filtered.map((employee) => {
              ...
              const privileged = employee.access_profile === "Inspetor";
              const accent = employeeAccent(employee);
              return <Surface key={employee.id} className="relative overflow-hidden p-4"><div className="absolute bottom-0 left-0 top-0 w-[3px]" style={{ background: active ? accent : "var(--text-4)" }} /><div className="flex min-w-0 items-center gap-3"><div className="flex h-11 w-11 shrink-0 items-center justify-center rounded-2xl text-sm font-black text-white" style={{ background: active ? accent : "#64748b" }}>{employee.full_name.charAt(0).toUpperCase()}</div><div className="min-w-0 flex-1"><p className="truncate text-sm font-black" style={{ color: "var(--text-1)" }}>{employee.full_name}</p><p className="mt-0.5 truncate text-xs" style={{ color: "var(--text-4)" }}>Mat. {employee.matricula} · {employee.sector}</p></div></div><div className="mt-3 flex items-center justify-between gap-2"><Badge variant="outline" className="max-w-[160px] truncate text-xs font-bold" style={{ color: accent, borderColor: `${accent}40`, background: `${accent}12` }}>{privileged ? "Inspetor · TI" : employee.access_profile}</Badge>{isAdmin && <div className="flex items-center gap-1"><Button variant="ghost" size="icon" onClick={() => openEdit(employee)}><Pencil className="h-4 w-4" /></Button>{!privileged && <Button variant="ghost" size="icon" className="text-red-500" onClick={() => setToDelete(employee)}><Trash2 className="h-4 w-4" /></Button>}</div>}</div></Surface>;
            })}
          </div>
        </>
      )}
        ...
        <DialogContent className="sm:max-w-2xl">
          ...
          <div className="space-y-4">
            ...
            <div className="grid gap-4 md:grid-cols-2">
              <div className="space-y-1.5 md:col-span-2"><Label htmlFor="full_name">Nome completo</Label><Input id="full_name" value={form.full_name} onChange={(event) => setForm({ ...form, full_name: event.target.value })} placeholder="Nome do funcionário" /></div>
              ...
              <div className="space-y-1.5"><Label>Setor</Label><Select value={form.sector} onValueChange={(value) => setForm({ ...form, sector: value })}><SelectTrigger><SelectValue /></SelectTrigger><SelectContent>{SETORES.map((sector) => <SelectItem key={sector} value={sector}>{sector}</SelectItem>)}</SelectContent></Select></div>
              <div className="space-y-1.5"><Label>Perfil de acesso</Label>{editingPrivileged ? <div className="flex h-9 items-center gap-2 rounded-md border px-3 text-sm" style={{ borderColor: "var(--border)", background: "var(--bg-surface-2)", color: "var(--text-3)" }}><LockKeyhole className="h-3.5 w-3.5 text-rose-500" /> Inspetor · TI</div> : <Select value={form.access_profile} onValueChange={(value) => setForm({ ...form, access_profile: value })}><SelectTrigger><SelectValue /></SelectTrigger><SelectContent>{PERFIS.map((profile) => <SelectItem key={profile} value={profile}>{profile}</SelectItem>)}</SelectContent></Select>}</div>
              <div className="space-y-1.5"><Label>Situação</Label><Select value={form.status} disabled={editingPrivileged} onValueChange={(value) => setForm({ ...form, status: value })}><SelectTrigger><SelectValue /></SelectTrigger><SelectContent>{SITUACOES.map((status) => <SelectItem key={status} value={status}>{status}</SelectItem>)}</SelectContent></Select></div>
            </div>
          </div>
