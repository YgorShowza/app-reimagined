  ...
  const save = async () => {
    ...
    try {
      for (let i = 0; i < drafts.length; i += 100) {
        await createCronogramaEntries(drafts.slice(i, i + 100).map(({ _key, ...row }) => row));
      }
      await Promise.all([
      ...
      toast.error(error?.message || "Não foi possível gerar o cronograma anual.");
    } finally {
      setSaving(false);
    }
  };
  ...
  return <Dialog open={open} onOpenChange={(v) => v ? onOpenChange(true) : close()}>
    <DialogContent className="sm:max-w-4xl max-h-[92vh] overflow-y-auto">
      ...
      {step === 1 && <div className="space-y-5 py-2">
        <div><Label>Ano</Label><div className="mt-2 flex items-center gap-2"><Button variant="outline" size="icon" onClick={() => setYear((y) => y - 1)}><ChevronLeft className="w-4 h-4" /></Button><Input className="w-32 text-center font-black" type="number" value={year} onChange={(e) => setYear(Number(e.target.value) || new Date().getFullYear())} /><Button variant="outline" size="icon" onClick={() => setYear((y) => y + 1)}><ChevronRight className="w-4 h-4" /></Button></div></div>
