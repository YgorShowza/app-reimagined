  ...
  return (
    <Dialog open={open} onOpenChange={(next) => next ? onOpenChange(true) : close()}>
      <DialogContent className="sm:max-w-xl">
        ...
        <DialogFooter><Button variant="outline" onClick={close} disabled={generating}>Cancelar</Button><Button onClick={generate} disabled={loading || !drafts.length || generating} className="bg-[#C8102E] text-white hover:bg-[#A00D24]">{generating ? <><Loader2 className="mr-2 h-4 w-4 animate-spin" /> Gerando...</> : <><ClipboardList className="mr-2 h-4 w-4" /> Gerar {drafts.length}</>}</Button></DialogFooter>
      </DialogContent>
    </Dialog>
  );
}
