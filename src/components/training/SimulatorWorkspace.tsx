...
import { ArrowRight, CheckCircle2, Play, Target, XCircle } from "lucide-react";
...
export function SimulatorWorkspace() {
  ...
  const scenariosQuery = useQuery({ queryKey: ["simulator-scenarios"], queryFn: listScenarios });
    ...
    return (
      <div className="mx-auto max-w-2xl space-y-5 py-6 text-center">
        ...
        <section className="rounded-3xl p-7" style={{ background: "var(--bg-surface)", border: "1px solid var(--border)", boxShadow: "var(--shadow-card)" }}>
          ...
          <div className="mt-4 inline-flex rounded-full px-4 py-2 text-sm font-black text-white" style={{ background: "#0ea5e9" }}>+{finished.xp} XP</div>
        </section>
    ...
    return (
      <div className="mx-auto max-w-2xl space-y-5 pb-10">
        <section className="rounded-[1.6rem] p-6 text-center" style={{ background: "linear-gradient(135deg,#171118,#2b0b13 52%,#111216)", border: "1px solid rgba(200,16,46,.26)" }}>
          ...
          <p className="mt-1 text-sm text-white/50">Cenários operacionais com decisão, feedback imediato e XP.</p>
        </section>
        ...
        <section className="rounded-2xl p-5 space-y-4" style={{ background: "var(--bg-surface)", border: "1px solid var(--border)", boxShadow: "var(--shadow-card)" }}>
          ...
          <div className="rounded-xl p-3 text-xs" style={{ background: "rgba(14,165,233,.08)", border: "1px solid rgba(14,165,233,.2)", color: "#0ea5e9" }}>Disponíveis para este filtro: <strong>{available.length}</strong> cenário(s). A sessão usa até 4 cenários e rende 20 XP ao concluir.</div>
  ...
  return (
    <div className="mx-auto max-w-2xl space-y-4 pb-10">
      ...
      <div className="h-2 overflow-hidden rounded-full" style={{ background: "var(--bg-surface-3)" }}><div className="h-full rounded-full bg-[#C8102E] transition-all" style={{ width: `${((current + 1) / session.length) * 100}%` }} /></div>
