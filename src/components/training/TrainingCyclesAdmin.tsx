...
import { AlertTriangle, CalendarClock, CheckCircle2, Clock3, Pencil, Plus, Search, Trash2 } from "lucide-react";
    ...
    <div className="mx-auto max-w-7xl space-y-5 pb-10">
      <section className="rounded-[1.5rem] p-5 md:p-6" style={{ background: "linear-gradient(135deg,#171118,#2b0b13 50%,#111216)", border: "1px solid rgba(200,16,46,.26)" }}>
        <div className="flex flex-col gap-4 md:flex-row md:items-end md:justify-between">
          ...
          {isAdmin && <Button onClick={openNew} className="bg-[#C8102E] text-white hover:bg-[#A00D24]"><Plus className="mr-2 h-4 w-4" /> Novo ciclo</Button>}
        </div>
      </section>
              ...
              <div className="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
                <div className="flex min-w-0 items-start gap-3">
                  ...
                  <div className="min-w-0">
                    <div className="flex flex-wrap items-center gap-2"><h2 className="font-black" style={{ color: "var(--text-1)" }}>{row.employee_name}</h2><span className="rounded-full px-2 py-0.5 text-[10px] font-black" style={{ background: conf.bg, color: conf.color }}>{row.status}</span></div>
                    <p className="mt-0.5 text-xs" style={{ color: "var(--text-4)" }}>Mat. {row.employee_matricula} · ciclo de {row.cycle_days} dias</p>
                    <div className="mt-3 flex flex-wrap gap-x-5 gap-y-1 text-xs" style={{ color: "var(--text-3)" }}><span>Último: <strong>{formatDate(row.last_training_date)}</strong></span><span>Atenção: <strong>{formatDate(row.window_start)}</strong></span><span>Vence: <strong style={{ color: conf.color }}>{formatDate(row.window_end)}</strong></span></div>
                    {row.observations && <p className="mt-2 text-xs" style={{ color: "var(--text-4)" }}>{row.observations}</p>}
                  </div>
                </div>
                {isAdmin && <div className="flex shrink-0 gap-1"><Button size="sm" variant="outline" onClick={() => openEdit(row)}><Pencil className="mr-1.5 h-3.5 w-3.5" /> Atualizar</Button><Button size="icon" variant="ghost" className="text-red-500" onClick={() => { if (confirm("Excluir este ciclo?")) remove.mutate(row.id); }}><Trash2 className="h-4 w-4" /></Button></div>}
              </div>
            </article>
          );
        })}
      ...
      {!schedules.isLoading && filtered.length === 0 && <section className="rounded-2xl p-12 text-center" style={{ background: "var(--bg-surface)", border: "1px solid var(--border)" }}><CalendarClock className="mx-auto h-10 w-10 opacity-25" /><p className="mt-3 font-bold" style={{ color: "var(--text-1)" }}>Nenhum ciclo encontrado.</p></section>}
