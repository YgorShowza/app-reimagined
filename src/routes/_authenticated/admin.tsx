...
function KPI({
  ...
  return (
    <Card className="relative overflow-hidden p-4 md:p-5">
      ...
      <div
        className="absolute -right-8 -top-8 h-24 w-24 rounded-full opacity-[.09]"
        style={{ background: accent }}
      />
      <div className="relative flex items-start justify-between gap-4">
        <div>
          <p className="text-[10px] font-black uppercase tracking-[.13em]" style={{ color: "var(--text-4)" }}>
            {label}
          </p>
          <p className="mt-3 text-3xl font-black tracking-tight" style={{ color: "var(--text-1)" }}>
            {value}
          </p>
          {sub && (
...
function AdminDashboard() {
  ...
  if (!data) {
    return (
      <Card className="p-8">
        <p style={{ color: "var(--text-3)" }}>Não foi possível carregar o dashboard.</p>
      </Card>
    );
  }
  ...
  return (
    <div className="mx-auto max-w-[1280px] space-y-5 pb-10">
      <section
        className="relative overflow-hidden rounded-[1.75rem] px-5 py-6 md:px-7 md:py-7"
        style={{
      ...
      >
        ...
        <div
          ...
          style={{ background: "radial-gradient(circle,rgba(200,16,46,.28),transparent 68%)" }}
        />

        <div className="relative flex flex-col gap-5 md:flex-row md:items-center md:justify-between">
          <div className="flex items-center gap-4">
            <div className="hidden sm:flex h-[68px] w-[118px] shrink-0 items-center justify-center rounded-2xl bg-white p-2 shadow-lg">
              <img src={LOGO_URL} alt="EMPAT" className="max-h-full max-w-full object-contain" />
            </div>
            <div>
              <p className="text-[10px] font-black uppercase tracking-[.24em]" style={{ color: "rgba(255,255,255,.50)" }}>
                {greeting}
              </p>
              <h1 className="mt-1 text-2xl font-black tracking-tight md:text-[2rem]" style={{ color: "#ffffff" }}>
                {first}
              </h1>
              <p className="mt-1 text-xs md:text-sm" style={{ color: "rgba(255,255,255,.52)" }}>
                {updatedAt} · Operação {year}
              </p>
            </div>
          </div>
          ...
          <div className="flex items-center gap-2 self-start md:self-auto">
            ...
            >
              ...
              <span className="text-sm font-black" style={{ color: "#ffffff" }}>
                {m.activeEmployees} ativos
              </span>
              <span style={{ color: "rgba(255,255,255,.24)" }}>·</span>
              <span className="text-sm font-black" style={{ color: priorityCount > 0 ? "#ff5a72" : "#86efac" }}>
                {priorityCount} em risco
              </span>
            </div>
            <button
              type="button"
              onClick={() => refetch()}
              disabled={isFetching}
              className="flex h-11 w-11 items-center justify-center rounded-2xl transition-opacity disabled:opacity-50"
              style={{ background: "rgba(255,255,255,.07)", border: "1px solid rgba(255,255,255,.13)", color: "#fff" }}
      ...
      <Card className="p-3 md:p-4">
        <div className="mb-3 flex items-center gap-2 px-1">
          ...
          <h2 className="text-xs font-black uppercase tracking-[.08em]" style={{ color: "var(--text-3)" }}>
            Acesso rápido
          </h2>
        </div>
        <div className="grid grid-cols-2 gap-2 sm:grid-cols-4 lg:grid-cols-8">
          {quick.map(({ to, label, icon: Icon, accent }) => (
            <Link key={to} to={to as any}>
              <div
                className="group relative flex h-full min-h-[82px] flex-col items-center justify-center gap-2 overflow-hidden rounded-xl p-3 text-center transition-[border-color,transform] duration-150 hover:-translate-y-0.5"
                style={{ background: "var(--bg-surface-2)", border: "1px solid var(--border)" }}
              >
                <div className="absolute -right-7 -top-7 h-16 w-16 rounded-full opacity-[.08]" style={{ background: accent }} />
                <div className="flex h-9 w-9 items-center justify-center rounded-xl" style={{ background: `${accent}12` }}>
      ...
      <div className="grid gap-4 lg:grid-cols-3">
        <Card className="overflow-hidden lg:col-span-2">
          <div className="flex items-center gap-2 p-4" style={{ borderBottom: "1px solid var(--border)" }}>
            ...
            <h2 className="text-sm font-black" style={{ color: "var(--text-1)" }}>
              Média por Setor
            </h2>
            <div className="ml-auto hidden items-center gap-3 sm:flex">
              {sectors.slice(0, 3).map((s, i) => (
                <span key={s.sector} className="flex items-center gap-1.5 text-[11px] font-semibold" style={{ color: "var(--text-4)" }}>
                  <span className="h-2 w-2 rounded-full" style={{ background: ["#e11d48", "#3b82f6", "#10b981"][i % 3] }} />
                  {s.sector} {s.executionRate}%
                </span>
              ))}
            </div>
          </div>
          ...
          {sectors.length === 0 ? (
            <div className="py-16 text-center">
              ...
              <p className="mt-1 text-xs" style={{ color: "var(--text-4)" }}>
                Os indicadores aparecem quando houver lançamentos no Cronograma.
              </p>
            </div>
          ) : (
            <div className="px-5 pb-5 pt-6">
              <div className="flex h-[210px] items-end justify-around gap-5 border-b border-dashed" style={{ borderColor: "var(--border)" }}>
                {sectors.map((s, i) => {
                  const color = ["#e11d48", "#3b82f6", "#10b981", "#f59e0b"][i % 4];
                  const height = Math.max(8, Math.min(100, s.executionRate));
                  return (
                    <div key={s.sector} className="flex h-full min-w-0 flex-1 flex-col items-center justify-end gap-2">
                      <span className="text-xs font-black" style={{ color }}>
                        {s.executionRate}%
                      </span>
                      <div className="w-full max-w-[78px] rounded-t-xl" style={{ height: `${height}%`, background: color, minHeight: 12 }} />
                      <span className="max-w-full truncate text-[11px] font-semibold" style={{ color: "var(--text-4)" }}>
                        {s.sector}
                      </span>
                    </div>
                  );
        ...
        <Card className="overflow-hidden" style={undefined as never}>
          ...
          <div className="max-h-[276px] space-y-2 overflow-y-auto p-3">
            {priority.slice(0, 6).map((r) => (
              <div key={r.employee.id} className="rounded-xl p-3" style={{ background: "var(--bg-surface-2)", border: "1px solid var(--border-subtle)" }}>
                <div className="flex items-center justify-between gap-3">
                  ...
                  <span
                    ...
                    style={{
                      color: r.level === "Alto" ? "#fb7185" : r.level === "Médio" ? "#f59e0b" : "#60a5fa",
                      background: r.level === "Alto" ? "rgba(225,29,72,.10)" : r.level === "Médio" ? "rgba(245,158,11,.10)" : "rgba(59,130,246,.10)",
                    }}
                  >
                    {r.level}
                  </span>
          ...
          <div className="px-4 pb-4">
            <Link to="/risco" className="flex items-center gap-1 text-xs font-bold" style={{ color: "var(--accent)" }}>
          ...
          <div className="grid gap-2 p-3 sm:grid-cols-2 lg:grid-cols-4">
            {leaders.map((e, i) => (
              <div
                ...
                style={{
                  background: i === 0 ? "rgba(200,16,46,.08)" : "var(--bg-surface-2)",
                  border: i === 0 ? "1px solid rgba(200,16,46,.28)" : "1px solid var(--border-subtle)",
                }}
              >
                <div className="text-2xl">{["🥇", "🥈", "🥉", "4º"][i]}</div>
                <p className="mt-2 truncate text-sm font-black" style={{ color: "var(--text-1)" }}>
                  {e.full_name}
                </p>
                <p className="mt-1 text-xs" style={{ color: "var(--text-4)" }}>
                  {e.sector}
                </p>
                <span className="mt-3 inline-flex rounded-lg px-2.5 py-1 text-xs font-black text-amber-500" style={{ background: "rgba(245,158,11,.10)" }}>
                  {e.points || 0} pts
                </span>
              </div>
            ))}
