...
import { UserRoundSearch, CheckCircle2, Clock3, XCircle, Target } from "lucide-react";
...
function IndividualPage() {
  ...
  const { data, isLoading } = useQuery({ queryKey: ["individual-snapshot", year], queryFn: () => getOperationalSnapshot(year) });
  const employees = useMemo(() => (data?.employees ?? []).filter((e) => e.status === "Ativo" && e.access_profile !== "Inspetor"), [data]);
  const [selected, setSelected] = useState("");
  const employee = employees.find((e) => e.id === selected) ?? employees[0];
  const cron = data?.cronograma.filter((e) => e.employee_id === employee?.id) ?? [];
  ...
  return <div className="mx-auto max-w-6xl space-y-5 pb-10">
    <div className="rounded-[1.5rem] p-5 md:p-6" style={{ background: "linear-gradient(135deg,#171118,#2b0b13 50%,#111216)", border: "1px solid rgba(200,16,46,.26)" }}><div className="flex items-center gap-2 text-[11px] uppercase tracking-[.2em] font-black text-white/40"><UserRoundSearch className="w-4 h-4" /> Desempenho individual</div><h1 className="mt-2 text-2xl md:text-3xl font-black text-white">Análise Individual</h1><p className="mt-1 text-sm text-white/50">Cruza cronograma e provas sem métricas fictícias.</p></div>
    <Card className="p-4"><label className="text-xs font-bold" style={{ color: "var(--text-4)" }}>Colaborador</label><div className="mt-2 max-w-md"><Select value={employee?.id || ""} onValueChange={setSelected}><SelectTrigger><SelectValue placeholder="Selecione..." /></SelectTrigger><SelectContent>{employees.map((e) => <SelectItem key={e.id} value={e.id}>{e.full_name} · {e.matricula} · {e.sector}</SelectItem>)}</SelectContent></Select></div></Card>
    {!employee ? <Card className="p-10 text-center"><p style={{ color: "var(--text-3)" }}>Nenhum colaborador ativo cadastrado.</p></Card> : <>
      <div className="grid grid-cols-2 lg:grid-cols-5 gap-3">{[["Planejados",cron.length,Target],["Realizados",realized,CheckCircle2],["Pendentes",pending,Clock3],["Aprovações",passed,CheckCircle2],["Média provas",avg,XCircle]].map(([l,v,I]: any) => <Card key={l} className="p-4"><div className="flex justify-between"><div><p className="text-[10px] uppercase font-black tracking-[.14em]" style={{ color: "var(--text-4)" }}>{l}</p><p className="mt-2 text-2xl font-black" style={{ color: "var(--text-1)" }}>{v}</p></div><I className="w-4 h-4" style={{ color: "var(--accent)" }} /></div></Card>)}</div>
      <div className="grid lg:grid-cols-2 gap-4"><Card className="overflow-hidden"><div className="px-4 py-3 font-bold text-sm" style={{ color: "var(--text-1)", borderBottom: "1px solid var(--border)" }}>Cronograma</div><div className="divide-y" style={{ borderColor: "var(--border)" }}>{cron.length ? cron.slice().sort((a,b) => (b.planned_date || "").localeCompare(a.planned_date || "")).map((e) => <div key={e.id} className="p-4"><div className="flex justify-between gap-3"><div><p className="font-semibold text-sm" style={{ color: "var(--text-1)" }}>{e.theme}</p><p className="text-xs mt-1" style={{ color: "var(--text-4)" }}>{formatDate(e.planned_date)}</p></div><span className="text-[10px] font-black" style={{ color: e.status === "Realizado" ? "#10b981" : e.status === "Pendente" ? "#f59e0b" : "#60a5fa" }}>{e.status}</span></div></div>) : <p className="p-6 text-sm" style={{ color: "var(--text-4)" }}>Sem lançamentos.</p>}</div></Card><Card className="overflow-hidden"><div className="px-4 py-3 font-bold text-sm" style={{ color: "var(--text-1)", borderBottom: "1px solid var(--border)" }}>Provas realizadas</div><div className="divide-y" style={{ borderColor: "var(--border)" }}>{attempts.length ? attempts.map((a) => <div key={a.id} className="p-4 flex justify-between gap-3"><div><p className="font-semibold text-sm" style={{ color: "var(--text-1)" }}>Nota {a.score}</p><p className="text-xs mt-1" style={{ color: "var(--text-4)" }}>{new Date(a.finished_at).toLocaleDateString("pt-BR")}</p></div><span className="text-[10px] font-black" style={{ color: a.passed ? "#10b981" : "#ef4444" }}>{a.passed ? "APROVADO" : "REPROVADO"}</span></div>) : <p className="p-6 text-sm" style={{ color: "var(--text-4)" }}>Sem tentativas.</p>}</div></Card></div>
    </>}
  </div>;
}
