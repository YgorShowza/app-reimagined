...
const actionLabel: Record<string,string> = {
  ...
  ISSUE_ACTIVATION_CODE:"Código de acesso emitido",
  REVOKE_ACTIVATION_CODE:"Código de acesso revogado",
...
function actionTone(action:string){
  if(action==="DELETE"||action==="REVOKE_ACTIVATION_CODE") return { color:"#ef4444", background:"rgba(239,68,68,.1)" };
  if(action==="UPDATE"||action==="PASSWORD_CHANGE") return { color:"#f59e0b", background:"rgba(245,158,11,.1)" };
  if(action==="LOGIN"||action==="ACTIVATE"||action==="ISSUE_ACTIVATION_CODE") return { color:"#3b82f6", background:"rgba(59,130,246,.1)" };
...
function AuditPage() {
  ...
  const query=useQuery({queryKey:["audit-logs"],queryFn:()=>listAuditLogs(500),staleTime:30_000});
  const data=query.data??[];
  const entities=useMemo(()=>["Todos",...Array.from(new Set(data.map(l=>l.entity))).sort()],[data]);
  const filtered=useMemo(()=>data.filter(l=>{if(entity!=="Todos"&&l.entity!==entity)return false;const q=search.toLowerCase().trim();return !q||[l.action,actionLabel[l.action],l.entity,l.entity_id,JSON.stringify(l.details)].some(v=>(v||"").toLowerCase().includes(q));}),[data,search,entity]);
  ...
  return <div className="segempat-governance-audit mx-auto max-w-6xl space-y-5 pb-10">
    ...
    <Card className="p-4"><div className="grid gap-3 md:grid-cols-[1fr_220px]"><div className="relative"><Search className="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4" style={{color:"var(--text-4)"}}/><Input value={search} onChange={(e)=>setSearch(e.target.value)} placeholder="Buscar ação, entidade, ID ou detalhe..." className="pl-10"/></div><Select value={entity} onValueChange={setEntity}><SelectTrigger><SelectValue/></SelectTrigger><SelectContent>{entities.map(e=><SelectItem key={e} value={e}>{e}</SelectItem>)}</SelectContent></Select></div></Card>

    <div className="space-y-2">{visible.map(l=>{const tone=actionTone(l.action);return <Card key={l.id} className="p-4"><div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between"><div className="min-w-0"><div className="flex flex-wrap items-center gap-2"><span className="text-[10px] font-black px-2 py-1 rounded-full" style={tone}>{actionLabel[l.action]||l.action}</span><p className="break-words font-bold text-sm" style={{color:"var(--text-1)"}}>{l.entity}</p></div><p className="mt-1 break-all text-xs" style={{color:"var(--text-4)"}}>ID: {l.entity_id||"—"}</p></div><p className="shrink-0 text-xs" style={{color:"var(--text-4)"}}>{fmtDate(l.created_at)}</p></div></Card>})}{!filtered.length&&<Card className="p-10 text-center"><History className="w-10 h-10 mx-auto opacity-30" style={{color:"var(--text-4)"}}/><p className="mt-3 font-bold" style={{color:"var(--text-1)"}}>Nenhum registro de auditoria encontrado.</p></Card>}</div>
